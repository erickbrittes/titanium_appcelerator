...
//Rare exceptions to indicate a bug in the _decarta code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_decarta = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_decarta]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.decarta.module"
