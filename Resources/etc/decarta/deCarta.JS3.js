...
deCarta.Core.Map = function(options){
    ...
    //The reason for this is the strong negative force 
    //generated by the combination of iOS Safari and IE
    //When combined, these two otherwise almost 
    //innocuous pieces of software can cause radiation 
    //poisoning, and have been shown to cause mild to severe
    //depression in programmers and laboratory rats. 
    ...
    //I realize it's not ideal, but it works. 
...
deCarta.Core.Configuration = {
    ...
    defaultTrafficConfig: 'traffic-usa1',
...
deCarta.Core.TrafficTileStore = function(opt){
    deCarta.Core.TileStore.call(this, opt);
    //a look up table providing the correct LLMAX values per zoom level.
    this._ll_LUT = deCarta.Core.Constants._ll_LUT;
    this.hostIdx = 0;
...
deCarta.Core.TrafficTileStore.prototype = {
...
deCarta.Core.TrafficTileStore.prototype = deCarta.Utilities.inherit(deCarta.Core.TrafficTileStore.prototype,   deCarta.Core.TileStore.prototype);
/**
 * @class
...
deCarta.Core.JSRequest = {
     ...
     * @param {function} requestInterceptor: function that has access to the full request before it is sent. 
     * if provided, what will be sent to the server will be the return value of the function.
     */
    send: function(xml, onSuccess, onFailure, getRaw, requestInterceptor){
        ...
        var completeXML = this.getRequestHeader() + xml + this.getRequestFooter();
        if (requestInterceptor && typeof requestInterceptor=='function') 
            completeXML = requestInterceptor(completeXML);

        if(deCarta.Core.Configuration.consolelogXML){
            console.log(completeXML);
        }

        var data  = encodeURIComponentUTF8(completeXML);
...
<xls:XLS version='1' xls:lang='en' xmlns:xls='http://www.opengis.net/xls' rel='4.5.1.sp02' xmlns:gml='http://www.opengis.net/gml'>
      ...
      <xls:Request maximumResponses='10' version='1.0' requestID='3332721' methodName='DetermineRouteRequest'>
        ...
        //console.log(criteria.trafficEnabled)
        if ( criteria.trafficEnabled =="true" || criteria.trafficEnabled === true) {
        ...
        var mapRequest = '';
        if(criteria.maneuverMaps){
            mapRequest = "<xls:RouteMapRequest><xls:Output orientation=\""+criteria.maneuverMapConfig.orientation+"\" format=\""+criteria.maneuverMapConfig.format+"\" width=\""+criteria.maneuverMapConfig.width+"\" height=\""+criteria.maneuverMapConfig.height+"\" /></xls:RouteMapRequest>";
        }
 ...
 * </pre>	 
    ...
    execute: function(criteria, callback){
        ...
        deCarta.Core.JSRequest.send(this.getXML(criteria), this.handleResponse.bind(this,callback), this.handleFailure.bind(this, callback), false, this.requestInterceptor.bind(this, criteria));
    ...
    //if necessary massages the request before it goes out. 
    requestInterceptor: function(criteria, request){
      //console.log('Making route request: ', criteria, request);
      //
      // Interceptor code goes here, like that
      //
      if ( criteria.trafficEnabled == "true" || criteria.trafficEnabled === true) {
        request = request.replace(deCarta.Core.Configuration.defaultConfig, deCarta.Core.Configuration.defaultTrafficConfig);

        console.log('Intercepted and modified to : ', request);
      }
      return request;
    },
